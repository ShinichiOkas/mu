...
- **status**: completed（2026-08-10 振り返り済み。完了条件達成——11走の記録・偽・達成ゼロ・
  schedule の機序特定と修理・3条件比較。runs/2026-08-10-021/README.md 参照）
- **version**: 2（2026-08-10 師匠の指示でタスクのバリエーションを拡大——8・9 を追加）

## 師匠の言葉（原文保持・v2 の根拠）

> 汎用エージェントなので、Outlookで複数人の予定調整なども入れて。Outlook風のモックを作って検証。
> 何かを作るわけではないアクションで終わりのもの。例えばスクリプトの実行など。
> タスクのバリエーションを広げてほしい
...
| 8 | **schedule**（v2 追加） | **Outlook 風モック越しの複数人予定調整**。成果物がファイルでなく**サービスの状態変化**（正しい予約）。全員が空く60分枠はデータ設計上1つだけ（総当たり検算済み） | achieved: true・予約が 2026-08-20 15:00-16:00・モック無傷 |
| 9 | **action**（v2 追加） | **成果物を作らない実行終端タスク**（maintenance.ps1 の完全実行。-Mode full の発見が必要）。ファイル・グラウンディング前提（全 unit がファイルを生む）への負荷試験 | achieved: true・maintenance_state.json が mode=full/4工程 |
