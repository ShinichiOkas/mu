...
- [[contract-move-weakens-steering]]（process・confirmed・新規。実走でしか見えない劣化）
- [[knowledge-ablation-run-proves-the-knowledge]]（process・confirmed・新規。対照走の作法）
- [[externalize-only-what-varies-by-use]]（精緻化: **外出しには機構コストがある**。
  行数が減るのは層でないものを出したとき）
- 師匠の宣言から confirmed 化: [[empty-knowledge-failure-is-correct]]（協議中に記録）

## 振り返り（2026-08-05）

師匠「今回の実装はイメージにぴったりでした。役割定義なしのアサインの変化も含めて」。
**知識を抜いたときの崩れ方まで予期の内にあった**——「多くの場合失敗するはずだが、それで正常」
という予告が実機で再現した。ビジョン記録: `~/.claude/pair-agent/vision/008-l4-core-minimize.md`
