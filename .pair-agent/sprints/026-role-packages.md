...
- 合意変更: 前提崩壊 0・より良い方法 1（MU_ROLES→既存 MU_ROLES_DIR への統合。追記済み）。
  差し戻し 0・逸脱 0。前置宣言5件は全て宣言どおり機能（特に「pjm は観測後に修正」の順序）。
- ビジョン照合（師匠原文）: 「パッケージ切り替えの形はイメージ通り。researchの苦戦も
  想定通りでした。やはり情報の範囲や振れ幅の大きいものの品質コントロールは難しい」
  → vision/026 に記録。
- Skill 登録（師匠承認・2件）: 「量・網羅性はテキスト規範では保証できない——構造に置く」
  （プロジェクト固有 / forming）／「プロンプト・役割・経路に触れたらクローズ前に実走スモーク」
  （ペア固有 / process / confirmed。起源は 025 の師匠指示）。
