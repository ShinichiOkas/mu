...
- [ ] **[重大] 検査器が成果物になる穴を塞ぐ** — 2026-08-06 追加（合意012 実走で観測。走行記録 [runs/2026-08-05-012/README.md](runs/2026-08-05-012/README.md)）
...
- [ ] **文書成果物の検査マーカーを言語非依存にする** — 2026-08-06 追加（合意012 観測③）
  - 「結論」で検査したが報告書は英語 `## Conclusion` で NG。内容は要件を満たしていた。
    ...
    または仕様側で成果物の言語を固定する
- [ ] **検証役に「作業場」を与える** — 2026-08-06 追加（合意012 観測④）
  - QA は `write_scope: own`（判定書のみ）のため中間検証メモが書けず、**権限拒否33回**を空転。
    文書の検証は「読んだ内容を書き出して突き合わせる」作業を伴う。
    規律（対象を書き換えない）は保ったまま scratch 領域を許す設計にする
