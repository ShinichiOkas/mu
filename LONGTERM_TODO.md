...
- [ ] **[重大] L2 が入力データを破壊する** — sales×gemma4 再走で、L2 が入力の `sales.csv` を
  自分のトイデータで上書き（412→71 bytes）。「この売上表から」という目的の対象データを壊した。
  実世界データなら不可逆。対策候補: 入力ファイルの保護（L4 が仕様で「入力は読み取り専用」を
  明示する／tools 側に書き込み禁止リストの機構を足す）。協議して決める
