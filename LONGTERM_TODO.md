...
- [ ] **[重大] PdM の答え仮定マーカー** — 入力の先頭5行の抜粋から「P003 は死に筋のはず」と
  推測し、受入検査の expect に焼き込んだ。マーカーが要求する以上、実装者は定義に反してでも
  P003 を報告書に載せるしかない（実際に載せ、自己矛盾した報告書が機械検査を通過した）。
  **答えを仮定したマーカーは代理指標より悪い**。対策候補: (a) roles/pdm.md に
  「expect には答え（計算結果・固有の判定結果）を置かない。構造（見出し・列・件数の形式）か、
  入力に実在する語だけを置く」 (b) 答えを置くなら実データ全量から計算させる
- [ ] **[重大] L2 が正直な FAIL 判定書を「未完了」と誤読する** — QA が ITEM 3: FAIL と
  正しく書いたのに、L2 Reflect は「報告書を直せ」と QA に要求し続けた（QA は write_scope で
  直せないし直すべきでない）。「判定を直しに行く」圧力＝014 の「検査器を直しに行く」の親戚。
  対策候補: QA タスクの成功条件（コード供給）に「FAIL/UNCERTAIN を含む判定書も、根拠が
  書かれていれば完成である」を明記
