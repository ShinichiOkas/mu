...
- [ ] **PdM（specify）が入力の実物を見ずに形式を発明する** — sales×12b で仕様が sales.csv の
  ヘッダーを2度発明（Product_Name… → 商品名,売上高,原価）し、実物との不一致が respec と
  入力破壊の起点になった。対策候補: specify の前にコード側で workdir 一覧＋既存ファイルの
  先頭を読んで PURPOSE に添える（005 の assess 証拠グラウンディングと同型）。協議して決める
- [ ] **[要協議・重大] PdM が検出した矛盾を人間に上げず独断解決する** — 難課題 H3（2026-08-01、
  [runs/2026-08-01-hard/README.md](runs/2026-08-01-hard/README.md)）で実観測。充足不能の目的
  （完全除去かつ1文字も変えない）に対し、PdM は矛盾を SPEC に明記した上で退化解（0バイト
  ファイル）を独断採用し、全層がその仕様に忠実に「達成」した。透明性（artifact 明示）は機能、
  欠けているのは「検出した矛盾は解決せず escalate する」規範/機構。対策候補:
  (a) specify プロンプトに充足不能時の escalate 規範（確率的） (b) QA の必須検査項目に
  「SPEC が目的の制約を弱めていないか（目的の原文と照合）」を追加 (c) 両方。協議して決める
- [ ] **入力保護は新規ファイル作成によるスコープ逸脱を防げない** — 難課題 H4 で実装者が
  保護対象ディレクトリに新規テストファイルを追加し、成果物のページ数がずれて escalate。
  軽微（正直に上がった）だが、保護の意味論（ファイル単位 vs ディレクトリ不変）は要検討
- [ ] **[要協議] QA の役割規律をツール制限で守る** — f1×12b r2 で QA が成果物を自分で修正して
  から合格判定（自己修正→自己承認）。role プロンプトの「実装しない」は確率的にしか効かない。
  対策候補: role=qa のタスクには書き込みを自タスクの出力ファイル（verdict.md）に限定した
  toolset を渡す（役割別ツール制限。approve / verify に続く第3の差し込み形）
