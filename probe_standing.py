...
# --- 責務文の版（`MU_DUTY` で選ぶ。既定は v4） ---------------------------------
#
# **なぜ切り替え可能にするか（合意039・師匠の指示）**: 責務文は 109 → 206 → 383 → 556字と
# 5倍に膨らんだ。しかも v4 で足したのは「検査器の名前・数えるものの定義・検算の手順」＝
# **責務ではなく手順書**である。北極星は「継続する責務を渡すと mu が自分で何をすべきか
# 決める」ことなので、手順書を渡したらそれは人間が目的を作っているのと同じになる。
# **肥大がポジティブに効いていたのかを切り分ける**——抜いた走が、その知識の効果を証明する
# （[[knowledge-ablation-run-proves-the-knowledge]]）。
#
# v3 と v4 の差は **「数えるものの限定」と「検算の要求」だけ**にしてある。
# 検査器の名前（`verify_readme.py`）は両方に入れる——規範ではなく計器の配線であり、
# 名前が動くと probe が持ち越せず、群の条件が揃わないため。
