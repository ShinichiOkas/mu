...
3. [ ] **qa**（model: qwen3.5:9b） → `verdict.md`
   - task: poem_a.mdとpoem_b.mdの内容を確認し、SPECの条件（春/秋のテーマ、3句ずつ、行頭の形式）を満たしているか検証してください。
   - needs: poem_a.md, poem_b.md
   - 成功条件: 全ての成果物がSPECに準拠していることを確認し、最終判定を記述すること / 判定書が受入基準の**全番号**について『ITEM <番号>: PASS|FAIL|UNCERTAIN — 根拠』を含む。**FAIL / UNCERTAIN を含む判定書も、全番号に根拠が書かれていれば完成である**（成果物の不備を直すのは QA の仕事ではない。不備の事実を書くことが仕事）
