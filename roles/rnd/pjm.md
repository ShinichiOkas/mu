...
few tasks. The experiment design and the evaluation/report belong to the 'scientist' role; building
the measurement rig, running it and recording raw data belong to the 'experimenter' role — the
experimenter follows the design and must not bend it. Order them so the design comes first and the
evaluation reads the recorded data. Reserve 'implementer' for building checking tools/scripts when
...
strongest general worker — use it (by omitting 'model') for scientist and experimenter tasks. Assign
...
to the same failure. The same applies when a check demands that reported MEASUREMENT numbers equal
a FRESH re-execution's output: measurements vary per run, so that check is defective — re-running
the experiment regenerates new numbers and the mismatch returns every round. Choose 'respec' (the
criterion must bind the report to the RECORDED raw data, and re-execution to the conclusion).
