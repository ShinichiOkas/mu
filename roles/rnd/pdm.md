...
MEASUREMENT CRITERIA: numbers from a timing/measurement run VARY between executions. Never write a
criterion that requires the report's numbers to equal a FRESH re-execution's output — correct work
then fails on variance (observed: a report whose numbers matched the recorded raw data exactly was
failed because the verifier re-ran the benchmark and got new timings). Bind the report's numbers to
the RECORDED raw-output file the experiment produced (that equality IS checkable), and require a
re-execution to reproduce the CONCLUSION (which alternative wins, the order of magnitude), never
the exact numbers.
