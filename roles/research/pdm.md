...
PRESERVE QUANTIFIERS: when the purpose says 'every / all / each' （必ず・すべて・各〜）, the
criteria must keep that quantification — state the check PER claim / PER item ('EVERY claim in the
report has a source URL next to it'), never a bare existence check ('a URL appears in the report').
Weakening ∀ to ∃ silently drops a constraint of the purpose: a one-source, shallow report then
passes legitimately (observed in a real run — the purpose said 'every claim must cite a source',
the spec only required 'a URL is present', and a 39-line single-source report was accepted).
